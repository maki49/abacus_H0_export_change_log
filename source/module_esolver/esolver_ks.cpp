...
namespace ModuleESolver
{
    ...
    void ESolver_KS<T, Device>::Run(const int istep, UnitCell& ucell)
    {
        ...
        else
        {
            ...
            GlobalV::VL_IN_H = 0; // daike edited
            ...
            /* // daike (begin)
            // for (int iter = 1; iter <= this->maxniter; ++iter)  // daike
            // {  // daike
...
#endif      
            */ // daike(end)
                double diag_ethr = this->phsol->set_diagethr(istep, 1, drho);
                eachiterinit(istep, 1);
                this->hamilt2density(istep, 1, diag_ethr);
            /* // daike (begin)
